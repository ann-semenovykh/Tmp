        ...
        private void DoSimulate( CheckEndOfModelling checkEndOfModelling )
        {
            ...
            do
            {
                ...
                if ( routineToExecute == null )
                    break;
                int[] list=new int[0];
                Object n= curnode.NodeRoutine.GetValueForVar(new CoreName("mark", list));
                //Выполняем в рутине ближайщее событие
                routineToExecute.EventCalendar.DoNextEvent();//проверка маркеров??
                if (curnode!=null)
                    //Visualize.Invoke(this, new VisualizeArgs(curnode,n));
