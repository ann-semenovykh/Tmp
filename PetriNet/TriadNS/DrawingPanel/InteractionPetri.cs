...
namespace DrawingPanel
{
    ...
    public class IPResize : InteractionPoint
    {
        ...
        public override void rePosition()
        {
            ...
            this.X1 = this.X +6; //толщина рамки выделения
            this.Y1 = this.Y +6;
        }
    ...
    public class CConnectionPoint : InteractionPoint
    {
        ...
        public override void rePosition()
        {
            ...
            this.X =  midX - Width / 2;// положение центра точки (откуда идет стрелка)
            this.Y =  midY - Width / 2;
            this.X1 = midX + Width / 2;
            this.Y1 = midY + Width / 2;
            if (Connectors == null)
                return;
            foreach (PetriLine connector in Connectors)
            {
                connector.RefreshPosition();
            }
            foreach (PetriMark mark in Marks)
        ...
        public override void Draw(Graphics g, int dx, int dy, float zoom)
        {
           // System.Drawing.Pen bluePen = new System.Drawing.Pen(System.Drawing.Color.Blue);
            //g.DrawLine(bluePen, (this.X + dx) * zoom, (this.Y + dy) * zoom, (this.X + dx + Width) * zoom, (this.Y + dy + Width) * zoom);//рисование в центре фигуры
