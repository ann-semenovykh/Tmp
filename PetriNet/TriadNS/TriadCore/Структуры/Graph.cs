...
namespace TriadCore
    ...
    public class Graph : ICreatable
        ...
        /// <param name="nodeToName">Конечная вершина </param>
        /// <param name="polusToName">Имя конечного полюса</param>
        public void AddEdge( CoreName nodeFromName, CoreName polusFromName, CoreName nodeToName, CoreName polusToName,bool reverse )
            {
        ...
        public static event EventHandler<VisualizeArgs> Visualize;
        /// <summary>
        /// Начать процесс имитации
        /// </summary>
        /// <param name="checkEndOfModelling">Проверка условия окончания моделирования</param>
        private void DoSimulate( CheckEndOfModelling checkEndOfModelling )  //РУЧНОЕ УПРАВЛЕНИЕ
        {
            systemTime = 0;
            ...
            VisualizeArgs va = new VisualizeArgs();
            Node prevnode;
            Node curnode = null;
            do
            {
                ...
                if ( routineToExecute == null )
                    ...
                    break;
                int[] list=new int[0];
                Object n = curnode.NodeRoutine.GetValueForVar(new CoreName("InitialNumMarks", list));
                va.CurNode = curnode;
                va.Num=(n != null) ? (int)n : 0;
                //Выполняем в рутине ближайщее событие
                routineToExecute.EventCalendar.DoNextEvent();//marks??
                if (curnode != null)
                    Visualize.Invoke(this, va);//new VisualizeArgs(curnode,n));
                if (va.EndModeling) break;
