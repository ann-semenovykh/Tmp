...
namespace TriadNSim.Ontology
{
    ...
    public class COWLOntologyManager
    {
        ...
        {
            OWLClassAssertionAxiom ax = ontology.getClassAssertionAxioms(indiv).iterator().next() as OWLClassAssertionAxiom;//transition!!
        ...
        public Routine CreateRoutine(OWLNamedIndividual indiv)
        {
            ...
            foreach(OWLNamedIndividual param in routParams)
            {
                ...
                VarType type = new VarType(GetTypeCode(paramType.getLiteral()), paramName.getLiteral());
        ...
        private bool TryComplete(SimulationInfo info, NetworkObject obj, OWLNamedIndividual indiv)
        {
            ...
            List<PetriLink> links = info.NodeLinks[obj];
            if (links.Count < nRequiredCount || poluses.Count < links.Count)
                return false;

            foreach (PetriLink link in links)
            {
                NetworkObject neighbor = (link.FromCP.Owner == obj ? link.ToCP.Owner : link.FromCP.Owner) as NetworkObject;
                if (neighbor.Routine != null)
                {
                    Polus polusName = link.FromCP.Owner == obj ? link.PolusTo : link.PolusFrom;
                    Polus neighborPolus = neighbor.Routine.GetPolus(polusName.Name);
        ...
        private bool CompleteNodeLink(SimulationInfo info, NetworkObject obj, List<Polus> poluses, int N, ref int nReqCount)
        {
            ...
            PetriLink link = links[N];
            Polus prevPolus = link.FromCP.Owner == obj ? link.PolusFrom : link.PolusTo;
        ...
        private List<Polus> GetPoluses(OWLNamedIndividual RoutineInvidiv)
        {
            ...
            OWLDataProperty dataPropIsInput=factory.getOWLDataProperty(IRI.create("http://www.psu.ru/ontologies/2009/4/ComputerNetwork.owl#IsInput"));
            OWLDataProperty dataPropIsOutput=factory.getOWLDataProperty(IRI.create("http://www.psu.ru/ontologies/2009/4/ComputerNetwork.owl#IsOutput"));
            ...
            foreach (OWLNamedIndividual pol in GetRoutinePoluses(RoutineInvidiv))
            {
                ...
                OWLLiteral isInput=((Set)map.get(dataPropIsInput)).iterator().next() as OWLLiteral;
                OWLLiteral isOutput=((Set)map.get(dataPropIsOutput)).iterator().next() as OWLLiteral;
                ...
                polus.IsInput = isInput.parseBoolean();
                polus.IsOutput = isOutput.parseBoolean();
                if (polus.UpperBounds.Count == 1) //!!!пока так
                {
                    for (int i = 0; i < polus.UpperBounds[0]; i++)
                    {
                        ...
                        p.IsInput = polus.IsInput;
                        p.IsOutput = polus.IsOutput;
                        res.Add(p);
                    }
                }
                else
        ...
        public List<IOWLClass> GetPetriNetElements()
        {
