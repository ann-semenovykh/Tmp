    ...
    public class NSModel : GraphBuilder
    {
        ...
        public override Graph Build()
        {
            ...
            foreach (NetworkObject node in SimInfo.Nodes)
            {
                ...
                string sType;
                if (node.Routine.AssemblyPath.Length == 0&& node.Routine.User!=true)
                {
                ...
                else
                {
                    ...
                    string sPath = "";
                    if (node.Routine.User == true)
                        sPath = Application.StartupPath + "\\" + node.Routine.Name + ".dll";
                    else
                        sPath = node.Routine.AssemblyPath;
                    assembly = Assembly.LoadFile(sPath);
                }
                Type RoutineType = assembly.GetType("TriadCore." + node.Routine.Name, true, true);//если файл был сохранен и повторно загружен, пользовательская рутина не загружается, если ее не принять заново
