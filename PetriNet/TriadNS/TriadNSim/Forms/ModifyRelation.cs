    ...
    public partial class ModifyRelation : Form
    {
        ...
        public ModifyRelation(PetriLink oLink)
        {
            ...
            if (oFrom != null && oTo != null)
            {
                ...
                if (oFrom.Routine != null || oTo.Routine != null)
                {
                    foreach (object obj in oFrom.drawingPanel.Shapes)
                    {
                        if (obj is PetriLink)
                        {
                            ...
                            if (link == oLink)
                                continue;
                            Polus polusName = link.FromCP.Owner == oFrom ? link.PolusFrom : (link.ToCP.Owner == oFrom ? link.PolusTo : null);
                            if (polusName != null && polusName.Name != string.Empty)
                                dictFrom[polusName.Name] = true;
                            polusName = link.FromCP.Owner == oTo ? link.PolusFrom : (link.ToCP.Owner == oTo ? link.PolusTo : null);
                            if (polusName != null && polusName.Name != string.Empty)
                                dictTo[polusName.Name] = true;
                        }
                    }
                }
                ...
                if (oLink.PolusFrom != null && oLink.PolusFrom.Name.Length > 0)
        ...
        private void textBox1_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (!char.IsDigit(e.KeyChar) && !char.IsControl(e.KeyChar))
                e.Handled = true;
            if (txtMult.Text == "") txtMult.Text = "0";
