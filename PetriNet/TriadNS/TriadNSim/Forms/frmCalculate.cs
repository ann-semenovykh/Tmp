...
namespace TriadNSim.Forms
{
    public partial class frmCalculate : Form
    {
        ...
        private void ToExcel()
        {
            ...
            List<PetriLink> lstLinks = new List<PetriLink>();
            foreach (BaseObject obj in drawingPanel.Shapes)
            {
                if (obj is NetworkObject)
                {
                    ...
                    lstLinks.Add(obj as PetriLink);
            }
            CoreName polus = new CoreName("p1");
            oGraph.DeclarePolusInAllNodes(polus);
            foreach (PetriLink link in lstLinks)
            {
                oGraph.AddEdge(new CoreName(link.FromCP.Owner.Name), polus, new CoreName(link.ToCP.Owner.Name), polus, true);//???
            }
